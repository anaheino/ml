...
n = int(0.9*len(shakespeare))
ending_point = len(shakespeare)
training_text = text[:n]
data = torch.tensor(encode(training_text), dtype=torch.long) # convert to tensor for handling
train_data = data[:n]
val_data = data[n:ending_point]
def toDevice(x, y):
    return x.to(device), y.to(device)

def get_batch(split):
    data = train_data if split == 'train' else val_data
    model_data_random = int(len(train_data) + len(val_data))
    ix = torch.randint(model_data_random - block_size, (batch_size,))
